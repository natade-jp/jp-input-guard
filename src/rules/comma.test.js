...
test("comma - format: 空や入力途中の値はそのまま返す", () => {
	const rule = comma();
...
test("comma - format: 3桁区切りカンマが整数部に付与される", () => {
	const rule = comma();
...
test("comma - format: 負数でも整数部にカンマが付く", () => {
	const rule = comma();
...
test("comma - format: 小数がある場合、整数部だけにカンマを付けて小数部はそのまま", () => {
	const rule = comma();
...
test("comma - format: 小数点が先頭にある形（.1）は整数部が空になり、そのまま '.1' になる", () => {
	const rule = comma();
...
test("comma - fromDataset: tigRulesComma が無ければ null", () => {
	const rule = comma.fromDataset({}, /** @type {any} */ (null));
	assert.equal(rule, null);
});

test("comma - fromDataset: tigRulesComma があれば comma ルールが返る", () => {
