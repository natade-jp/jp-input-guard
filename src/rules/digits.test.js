...
test("validate: 入力途中（空文字）は何もしない（errors/revert なし）", () => {
...
test("validate: 入力途中（-）は何もしない（errors/revert なし）", () => {
...
test("validate: int/frac 両方 overflow（block なし）ならエラーが2件積まれる", () => {
	const rule = digits({
		int: 2,
		frac: 2,
...
test("validate: int/frac 両方 overflow でも、block が優先され requestRevert して errors は積まない（int が先）", () => {
	const rule = digits({
		int: 2,
		frac: 2,
...
test("validate: countLeadingZeros=false だと全部ゼロ '000' は 1桁扱いでOK", () => {
...
test("fix: frac 未指定（省略）の場合は dot があっても小数を落として返す", () => {
	const rule = digits({ int: 10 }); // frac は指定しない
	assert.equal(rule.fix("12.34", createCtx()), "12");
});

test("fix: forceFracOnBlur=true なら dot が無い値でも frac 桁の .00 を付ける", () => {
	const rule = digits({ frac: 2, forceFracOnBlur: true });
	assert.equal(rule.fix("12", createCtx()), "12.00");
});

test("fix: forceFracOnBlur=true なら '12.' のように小数部が空でも 0 埋めする", () => {
	const rule = digits({ frac: 2, forceFracOnBlur: true });
	assert.equal(rule.fix("12.", createCtx()), "12.00");
});

test("fix: forceFracOnBlur=true なら '12.3' のように小数部が短い場合も 0 埋めする", () => {
	const rule = digits({ frac: 2, forceFracOnBlur: true });
	assert.equal(rule.fix("12.3", createCtx()), "12.30");
});

test("fix: round の境界（小数側の繰り上げ連鎖） 1.995 -> 2.00", () => {
	const rule = digits({ frac: 2, fixFracOnBlur: "round" });
...
test("fromDataset: 不正な enum/number が来ても落ちずに rule が作れる（デフォルトにフォールバック）", () => {
	const dataset = {
		tigRulesDigits: "1",
